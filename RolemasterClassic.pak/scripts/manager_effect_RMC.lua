...
function onEffectAddStart(rEffect)
	...
	if rEffect.sUnits == "minute" then
		rEffect.nDuration = rEffect.nDuration * 6;
...
function onEffectActorEndTurn(nodeActor, nodeEffect)
	...
	for _,sEffectComp in ipairs(aEffectComps) do
		...
		elseif rEffectComp.type == "DMGO" or rEffectComp.type == "Bleeding" or rEffectComp.type == "REGEN" then
			local nActive = DB.getValue(nodeEffect, "isactive", 0);
			if nActive == 2 then
				if rEffectComp.type == "REGEN" and (ActorManagerRMC.getWoundPercent(rActor) >= 1) then 
					break;
				end
...
function applyOngoingDamageAdjustment(nodeActor, nodeEffect, rEffectComp)
	...
	if rEffectComp.type == "REGEN" then
		local nPercentWounded = ActorManagerRMC.getWoundPercent(rTarget);
		...
		local rAction = {};
		if rEffectComp.type == "Bleeding" then
...
function evalAbilityHelper(rActor, sEffectAbility)
	...
	local nAbility = nil;
	if sTag == "STR" then
		nAbility = ActorManagerRMC.getAbilityBonus(rActor, "strength");
	elseif sTag == "DEX" then
		nAbility = ActorManagerRMC.getAbilityBonus(rActor, "dexterity");
	elseif sTag == "CON" then
		nAbility = ActorManagerRMC.getAbilityBonus(rActor, "constitution");
	elseif sTag == "INT" then
		nAbility = ActorManagerRMC.getAbilityBonus(rActor, "intelligence");
	elseif sTag == "WIS" then
		nAbility = ActorManagerRMC.getAbilityBonus(rActor, "wisdom");
	elseif sTag == "CHA" then
		nAbility = ActorManagerRMC.getAbilityBonus(rActor, "charisma");
	elseif sTag == "LVL" then
		nAbility = ActorManagerRMC.getAbilityBonus(rActor, "level");
	elseif sTag == "PRF" then
		nAbility = ActorManagerRMC.getAbilityBonus(rActor, "prf");
	else
		nAbility = ActorManagerRMC.getAbilityScore(rActor, sTag:lower());
	end
				...
				for kEffectComp,sEffectComp in ipairs(aEffectComps) do
					...
					-- Compare other attributes
						...
						if comp_match then
							nMatch = kEffectComp;
							if nActive == 1 then
								rEffectComp.effectdesc = sDescription;
...
function summarizeEffect(sUser, sIdentity, nodeCT, rNewEffect, bShowMsg, nEffectGMOnly)
	local rActor = ActorManager.resolveActor(nodeCT);
	local sEffectName = rNewEffect.sName;
	
	if hasEffect(rActor, sEffectName) then
		-- Iterate through effects
		for _,vEffect in pairs(DB.getChildren(ActorManager.getCTNode(rActor), "effects")) do
			-- Check active
			if DB.getValue(vEffect, "isactive", 0) ~= 0 then
				local sLabel = DB.getValue(vEffect, "label", "");
				local nDuration = DB.getValue(vEffect, "duration", 0);

				if string.find(sLabel, rNewEffect.sName) and not string.find(sLabel, "Penalty:") then
					local nNewEffectDuration = rNewEffect.nDuration or 0;
					local nNewDuration = nDuration + nNewEffectDuration;
					...
					msg.text = msg.text .. " changed from " .. nDuration .. " to " .. nNewDuration .. " rounds";
		...
		rNewEffect.nGMOnly = nEffectGMOnly;
		EffectManager.addEffect(sUser, sIdentity, nodeCT, rNewEffect, bShowMsg);
	end
end

function notifyImmunity(sUser, sIdentity, nodeCT, rNewEffect, bShowMsg, nEffectGMOnly)
	-- Build output message
	local msg = {font = "msgfont", icon = "roll_effect"};
	msg.text = "Effect ['" .. rNewEffect.sName .. "'] ";
	msg.text = msg.text .. "-> [to " .. ActorManager.getDisplayName(nodeCT) .. "]";
	msg.text = msg.text .. " [IMMUNE]";
...
function checkConditional(rActor, nodeEffect, aConditions, rTarget, aIgnore)
	...
	for _,v in ipairs(aConditions) do
		...
		if sLower == DataCommon.healthstatusfull then
			local nPercentWounded = ActorManagerRMC.getWoundPercent(rActor);
			if nPercentWounded > 0 then
				bReturn = false;
				break;
			end
		...
		elseif StringManager.contains(DataCommon.conditionaltags, sLower) then
			...
			if sAlignCheck then
				if not ActorManagerRMC.isAlignment(rActor, sAlignCheck) then
					bReturn = false;
					break;
				end
			elseif sSizeCheck then
				if not ActorManagerRMC.isSize(rActor, sSizeCheck) then
					bReturn = false;
					break;
				end
			elseif sTypeCheck then
				if not ActorManagerRMC.isCreatureType(rActor, sTypeCheck) then
					bReturn = false;
					break;
				end
