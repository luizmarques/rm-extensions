...
function onInit()
	TableManager.performRoll = performRoll;
	TableManager.getTableDice = getTableDice;
	TableManager.getTableDiceFromRowData = getTableDiceFromRowData;
	TableManager.onTableRoll = onTableRoll;
	TableManager.processTableRoll = processTableRoll;
...
-- Modified Section Start
		rTableRoll.aDice, rTableRoll.nMod, rTableRoll.sDiceType = TableManager.getTableDice(rTableRoll.nodeTable);
...
-- Modified Section Start
	if (rTableRoll.sDiceType or "") ~= "" then
		rRoll = ActionRMDice.setupRoll("table", rTableRoll.sDiceType, rRoll.nMod, rRoll.sDesc, rTableRoll.bSecret);
	end
	rRoll.sNodeTable = DB.getPath(rTableRoll.nodeTable);
-- Modified Section End
...
-- Modified Section End
	...
	if #aDice == 0 then
-- Modified Section Start
		aDice, nMod, sDiceType = TableManager.getTableDiceFromRowData(nodeTable);
-- Modified Section End
	else
...
-- Modified Section Start
	if rRoll.sDiceType then
		rMessage, rRoll = ActionRMDice.processRoll(rSource, rTarget, rRoll);
	end

	if rMessage or not rRoll.sDiceType then
-- Modified Section End
...
-- Modified Section Start
		rTableRoll.aDice, rTableRoll.nMod, rTableRoll.sDiceType = TableManager.getTableDice(nodeTable);
-- Modified Section End
	end
