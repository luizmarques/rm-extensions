...
function modDamage(rSource, rTarget, rRoll)
	...
	if rSource then
		...
		for _,vClause in ipairs(rRoll.clauses) do
			local nBonusStat, nBonusEffects = ActorManagerRMC.getAbilityEffectsBonus(rSource, vClause.stat);
		...
		local aEffects, nEffectCount = EffectManagerRMC.getEffectsBonusByType(rSource, "DMG", true, aAttackFilter, rTarget);
		...
		local aEffects = EffectManagerRMC.getEffectsByType(rSource, "DMGTYPE", {});
		local aAddTypes = {};
		...
		if EffectManagerRMC.hasEffect(rSource, "Incorporeal") then
			bEffects = true;
	...
	local bMax = ModifierManager.getKey("DMG_MAX");
	if bMax then
		table.insert(aAddDesc, "[MAX]");
	end
	local bHalf = ModifierManager.getKey("DMG_HALF");
	if bHalf then
...
function getReductionType(rSource, rTarget, sEffectType)
	local aEffects = EffectManagerRMC.getEffectsByType(rTarget, sEffectType, {}, rSource);
...
function getDamageAdjust(rSource, rTarget, nDamage, rDamageOutput)
	...
	local bIncorporealSource = EffectManagerRMC.hasEffect(rSource, "Incorporeal", rTarget);
	local bIncorporealTarget = EffectManagerRMC.hasEffect(rTarget, "Incorporeal", rSource);
	...
	local nDTMod, nDTCount = EffectManagerRMC.getEffectsBonus(rTarget, {"DT"}, true);
	if nDTMod > 0 then
...
function decodeDamageText(nDamage, sDamageDesc)
	...
	if type(nDamage) == "string" then
		nDamage = tonumber(nDamage);
	end
...
function applyDamage(rSource, rTarget, bSecret, sDamage, nTotal)
	...
	if sTargetType ~= "charsheet" and sTargetType ~= "npc" then
		return;
	end

	local nTotalHP, nWounds, nConstitution = 1;
	if sTargetType == "charsheet" then
		nTotalHP = DB.getValue(nodeTarget, "hits.max", 0);
		nWounds = DB.getValue(nodeTarget, "hits.damage", 0);
		nConstitution = DB.getValue(nodeTarget, "abilities.constitution.temp_current", 1);
	else
		nTotalHP = DB.getValue(nodeTarget, "hits", 0);
		nWounds = DB.getValue(nodeTarget, "damage", 0);
		if string.lower(OptionsManager.getOption("NCON")) == string.lower(Interface.getString("option_val_co_npc")) then
			nConstitution = nTotalHP;
		elseif string.lower(OptionsManager.getOption("NCON")) == string.lower(Interface.getString("option_val_co_50")) then	
			nConstitution = 50;
		end		
	end
	...
	local _,sOriginalStatus = ActorManagerRMC.getWoundPercent(rTarget);
	...
	if rDamageOutput.sType == "recovery" then
		...
		if nWounds <= 0 then
			...
			if sTargetType == "charsheet" and sClassNode then
	...
	-- Damage
		...
		if rSource and rTarget and rTarget.nOrder then
			...
			local aTargetedDamage = EffectManagerRMC.getEffectsBonusByType(rSource, {"DMG"}, true, rDamageOutput.aDamageFilter, rTarget, true);
...
--				nWounds = nTotalHP;
				...
				for _,v in pairs(DB.getChildren(nodeTargetCT, "effects")) do
					...
					if (nActive == 1) then
						...
						for i = 1, #aEffectComps do
							local rEffectComp = EffectManagerRMC.parseEffectComp(aEffectComps[i]);
	...
	-- Check for Unconscious and Dead conditions
	if nWounds < nTotalHP then
		if EffectManagerRMC.hasEffect(rTarget, "Unconscious") then
			EffectManager.removeEffect(ActorManager.getCTNode(rTarget), "Unconscious");
		end
	else
		local nBelowZero = nWounds - nTotalHP;
		local nodeEntry = ActorManager.getCTNode(rTarget);
		local nEffectInit = DB.getValue(nodeEntry, "initresult", 1) - 1;
		if nConstitution and nBelowZero >= nConstitution then
			nWounds = nConstitution + nTotalHP;
			if not EffectManagerRMC.hasEffect(rTarget, "Dead") then
				EffectManager.addEffect("", "", nodeEntry, { sName = "Dead", nDuration = 0, nInit = nEffectInit }, true);
			end
			if EffectManagerRMC.hasEffect(rTarget, "Unconscious") then
				EffectManager.removeEffect(nodeEntry, "Unconscious");
			end
		else
			if not EffectManagerRMC.hasEffect(rTarget, "Unconscious") and not EffectManagerRMC.hasEffect(rTarget, "Dead")  then
				EffectManager.addEffect("", "", nodeEntry, { sName = "Unconscious", nDuration = 0, nInit = nEffectInit }, true);
			end
		end
	end
	...
	-- Set health fields
	if sTargetType == "charsheet" then
		DB.setValue(nodeTarget, "hits.damage", "number", nWounds);
	else
		DB.setValue(nodeTarget, "damage", "number", nWounds);
	end

	-- Check for status change
	local bShowStatus = false;
	if ActorManager.isFaction(rActor, "friend") then
		bShowStatus = not OptionsManager.isOption("SHPC", "off");
	else
		bShowStatus = not OptionsManager.isOption("SHNPC", "off");
	end
	if bShowStatus then
		local _,sNewStatus = ActorManagerRMC.getWoundPercent(rTarget);
