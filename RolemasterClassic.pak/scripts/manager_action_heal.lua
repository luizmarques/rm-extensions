...
function getRoll(rActor, rAction)
	...
	for _,vClause in pairs(rRoll.clauses) do
		...
		local sAbility = Rules_Stats.GetDisplayNameFromAbbr(vClause.stat);
		if sAbility then
...
function modHeal(rSource, rTarget, rRoll)
	...
	if rSource then
		...
		local nEffectCount;
		aAddDice, nAddMod, nEffectCount = EffectManagerRMC.getEffectsBonus(rSource, {"HEAL"});
		...
		for sAbility, sAbilityMult in rRoll.sDesc:gmatch("%[MOD: (%w+) %((%w+)%)%]") do
			local nBonusStat, nBonusEffects = ActorManagerRMC.getAbilityEffectsBonus(rSource, DataCommon.ability_stol[sAbility]);
