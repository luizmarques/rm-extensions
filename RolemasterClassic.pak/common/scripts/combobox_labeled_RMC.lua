...
local sListFont = "chatfont";
local sListSelectedFont = "narratorfont";
local sListFrame = "";
...
function onInit()
	...
	if (sName or "") ~= "" then
		...
		ctrlButton = window.createControl("combobox_button_RMC", sButton);
...
function showList()
	...
	if not ctrlList and (sName or "") ~= "" then
		...
		ctrlList = window.createControl("combobox_list_sorted_RMC", sList);
		...
		ctrlScroll = window.createControl("combobox_scrollbar_RMC", sListScroll);
