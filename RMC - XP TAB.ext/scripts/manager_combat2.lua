...
function restHours(nHours)
	...
	for _, nodeCT in pairs(CombatManager.getCombatantNodes()) do
		if not EffectManager.hasCondition(nodeCT, "Dead") then
			...
			local sRace = StringManager.trim(string.lower(DB.getValue(nodeCT, "race", "")));
			local tElfRaces = {
				["fair elf"] = true, ["high elf"] = true, ["wood elf"] = true,
				["dark elf"] = true, ["aquatic elf"] = true, ["grey elf"] = true
			};

			if nHours >= 8 or (nHours >= 2 and Rules_PC.IsElf(ActorManager.getCreatureNode(nodeCT))) then
...
function addWoundEffects(nodeTarget, woundEffects, description)
	...
	if bUseConditionalEffects then  -- Use conditional effects
		...
		if woundEffects.ConditionalWound then
			if not EffectManagerRMC.hasCondition(nodeTarget, woundEffects.ConditionalWound) then
		...
		if woundEffects.ConditionalEffect then
			if not EffectManagerRMC.hasCondition(nodeTarget, woundEffects.ConditionalEffect) then
		...
		if woundEffects.ConditionalEffect2 then
			if not EffectManagerRMC.hasCondition(nodeTarget, woundEffects.ConditionalEffect2) then
		...
		if woundEffects.ConditionalEffect3 then
			if not EffectManagerRMC.hasCondition(nodeTarget, woundEffects.ConditionalEffect3) then
		...
		if woundEffects.Wound then
			if not EffectManagerRMC.hasCondition(nodeTarget, woundEffects.Wound) then
		...
		if woundEffects.Effect then
			if not EffectManagerRMC.hasCondition(nodeTarget, woundEffects.Effect) then
		...
		if woundEffects.Effect2 then
			if not EffectManagerRMC.hasCondition(nodeTarget, woundEffects.Effect2) then
		...
		if woundEffects.Effect3 then
			if not EffectManagerRMC.hasCondition(nodeTarget, woundEffects.Effect3) then
...
function handleCTUpdateOwners()
	...
	for _,v in pairs(CombatManager.getCombatantNodes()) do
		local nodePC = CombatManager2.getPCLinkNode(v);
		if nodePC then
			CombatManager2.updateOwner(v, nodePC.getOwner());
		end
	end
end
